...
$(".side-menu").on(
		...
		function(e) {
			...
			//$(this).css({"background":"#63B8FF","outline":"none"});
			$(this).css({"background":"#33AECC","outline":"none"});
		});
...
/* 添加tab */
function addTab(cur) {
	var prev_all = tabWidth($(cur).prevAll()), next_all = tabWidth($(cur).nextAll());
...
function navChange() {
	...
	if (!$(this).hasClass("active")) {
		...
		$(".side-menu").find('span').each(function(){
			...
			if(s.indexOf(html)>-1){
				$(this).css('color','white');
				$(this).parent("a").css('background','#63B8FF');
			}
			else{
				$(this).css('color','black');
...
$(function() {
	...
	var APP_CODE = $("#APP-CODE").val();
	$.post(CONTEXT_PATH + "/system/menus",{appcode : APP_CODE},function(data){
		if (data.status == "1") {
			var uiMenus = [];
			for(var i=0; i<data.data.length;i++){
				uiMenus.push(change2UIMenu(data.data[i]));
			}
